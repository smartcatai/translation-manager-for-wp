...
class Callback extends QueueAbstract {
	...
	public function update_statistic( $item ) {
		...
		try {
			if ( $statistics ) {
				Logger::event( "callback", "Queue update_statistic '{$statistics->get_document_id()}'");
			...
			if ( $e->getResponse()->getStatusCode() == 404 ) {
				...
				Logger::event( "callback", "Deleted '{$statistics->get_document_id()}'");
			} else {
	...
	protected function task( $item ) {
		...
		try {
			...
			if ( $statistics && $statistics->get_error_count() < 360 ) {
				...
				Logger::event( "callback", "New {$statistics->get_error_count()} try '{$statistics->get_document_id()}'");
				return $item;
			}
